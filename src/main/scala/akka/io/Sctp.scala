...
import akka.actor._
import com.sun.nio.sctp.{ SctpChannel, SctpStandardSocketOptions, Association }
...
object Sctp extends ExtensionId[SctpExt] with ExtensionIdProvider {
  ...
  /**
   * Sending this command to the connection actor will disable reading from the SCTP
   * socket. SCTP flow-control will then propagate backpressure to the sender side
   ...
   */
  final case class Bound(localAddresses: Set[InetSocketAddress]) extends Event
